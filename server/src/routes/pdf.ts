...
router.get('/', async (req, res) => {
  ...
  res.setHeader('Content-Disposition', `attachment; filename=MyMoney_Report_${month}.pdf`);
  doc.pipe(res);

  // Title
  doc.fontSize(22).fillColor('#10b981').text('MyMoney', { align: 'center' });
  ...
  // Footer
  doc.fontSize(7).fillColor('#9ca3af').text(`Generated by MyMoney on ${new Date().toLocaleDateString()}`, 50, 780, { align: 'center' });
