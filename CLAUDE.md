...
- **Reconcile module** (`client/src/pages/Reconcile.tsx`, route `/reconcile`) — fixes tracked-vs-actual balance drift. "Reconcile" an account = enter its real balance → one balancing transaction is created in **"Balancing - Income"** (tracked too low) or **"Balancing - Expense"** (tracked too high). "Itemize" carves a remembered amount out of a balancing entry: partial → `editTransaction` shrinks the entry + `addTransaction` for the piece; full → `editTransaction` recategorizes the whole entry. No split-endpoint, no schema change. Balancing entries **are counted** in Dashboard income/expense (unlike entrusted — this is real money, just uncategorized). The 2 balancing categories are protected + seeded + auto-created on the page.
- **Entrusted Fund module** (`client/src/pages/EntrustedFund.tsx`, route `/entrusted`) — tracks money other people entrust to the user for shared plans. Backed by the `entrusted_funds` table + a nullable `entrusted_fund_id` column on `transactions`. Three protected categories: **"Entrusted Funds"** (income — a contribution; contributor name in `notes` first line), **"Entrusted Spend"** (expense — spent on the shared purpose), **"Entrusted Return"** (expense — money given back to a contributor; contributor name in `notes` first line, netted against their contribution). All are real transactions (account balances stay correct) but the **three entrusted categories are excluded** from Dashboard income/expense/savings and the analytics aggregations (`getTodayTotal/Weekly/Monthly`, category breakdown, trend) — see `exclEntrusted()` in repository.ts and `excludeEntrusted()`/`ENTRUSTED_CATEGORY_NAMES` in server analytics.ts. The module manages its own entries via in-modal CRUD (never routes to `/add`). **Per-fund `members`** (JSON-array TEXT column on `entrusted_funds`, exposed to clients as `string[]`) is a roster of expected contributors registered without an amount — they show as ⏳ pending in the **monthly roster** (`<input type=month>` per expanded fund) until they actually contribute; the roster unions `members` ∪ contribution-derived names. Server route (de)serializes members JSON↔array (`withMembersArray`/`membersToJson`); native repo uses `parseMembers` + `JSON.stringify`. Monthly report (CSV via `utils/saveFile.ts`) lists each contributor's status for the chosen month.
